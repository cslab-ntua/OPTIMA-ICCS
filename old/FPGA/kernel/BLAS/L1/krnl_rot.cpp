...
static void rot(hls::stream< float>& Xin, hls::stream< float>& Yin,hls::stream< float>& Xout,hls::stream< float>& Yout, int N, float C, float S) {

execute:

    for (int i = 0; i < N; i++) {
	#pragma HLS pipeline II=1
    	Xout << C*Xin.read()+S*Yin.read();
    	Yout << C*Yin.read()-S*Xin.read();
    }
}


extern "C" {
void krnl_rot(int N,float* X,int incx,  float* Y, int incy, float C, float S) {
