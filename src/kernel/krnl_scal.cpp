...
static void scal(hls::stream< float>& inStream, hls::stream< float>& outStream, int size,float SA) {

execute:
#pragma HLS pipeline II=1
    for (int i = 0; i < size; i++) {

        outStream << (inStream.read()*SA);
    }
}
...
extern "C" {
void krnl_scal(int size,float* in,int incx,  float* out, int incy, float SA) {
...
#pragma HLS dataflow
    ...
    scal(inStream, outStream, size,SA);
