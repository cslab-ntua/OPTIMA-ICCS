...
// set DATA_SIZE =N;
	DATA_SIZE=N;
	   ...
	   cl::Kernel krnl;
	    ...
	    posix_memalign((void **)&result_host, 4096, sizeof(double));
	    std::cout<< *result_host <<" should be 0"<<std::endl;
	    *result_host =0;
	    std::cout<< *result_host <<" should be 0"<<std::endl;

	    //verify
	    double sw_result=((double)alpha);
...
void OOPS_rotm( const int N, float *X, const int incX, float *Y, const int incY, const float *P,char* arg1){
    ...
    std::vector<float, aligned_allocator<float> > Sx_host(DATA_SIZE*incX);
    std::vector<float, aligned_allocator<float> > Sy_host(DATA_SIZE*incY);
	std::vector<float, aligned_allocator<float> > Params_host(5);
	...
	for(int i=0;i<5;i++){
	    ...
	    for (int i = 0; i < N; i++) {
	        ...
	        x_results[i]=sh11*X[x_index]+Y[y_index];
