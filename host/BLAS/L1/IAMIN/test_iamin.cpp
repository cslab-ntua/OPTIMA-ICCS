int OOPS_iamin(const int N, const int NCU, const int MAX_CUS, const float  *X, const int incX){
		cl_int err;
		std::string cu_id;
		std::string krnl_name = "krnl_iamin";
		...
		int base_idx = N/NCU;
		float min_val;
		int *partial_min_idx = (int*)OOPS_malloc(sizeof(int)*NCU);


		for (int i = 0; i < NCU; i++)
		{
			cu_id = std::to_string(i+1);
			std::string krnl_name_full = krnl_name + ":{" + "krnl_iamin_" + cu_id + "}";
		...
		for (int i = 0; i < NCU; i++) {
			if(i!=NCU-1){
				...
				OCL_CHECK(err, result[i] = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY ,
						sizeof(int),(void*)&partial_min_idx[i], &err));
		}
		...
		for (int i =0; i < NCU; i++)
		{
			...
			if(min_val > X[(base_idx*i) + partial_min_idx[i]])
			{
				min_val = X[(base_idx*i) + partial_min_idx[i]];
				min_idx = (base_idx*i) + partial_min_idx[i];
			}
		}
		free(partial_min_idx);
